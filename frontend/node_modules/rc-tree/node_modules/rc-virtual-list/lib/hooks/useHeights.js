...
function useHeights(getKey, onItemAdd, onItemRemove) {
  ...
  var heightsRef = (0, React.useRef)(new Map());
  var instanceFuncRef = (0, React.useRef)(new Map());

  function getInstanceRefFunc(item) {
    var key = getKey(item);

    if (!instanceFuncRef.current.has(key)) {
      instanceFuncRef.current.set(key, function (instance) {
        var origin = instanceRef.current.get(key);
        instanceRef.current.set(key, instance); // Instance changed
